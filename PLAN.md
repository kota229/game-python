...
- [x] 設計書のステージ一覧に従い、合計**31ステージ**を実装（block10/bridge5/fill8/free8）。
      全解答を stages.test.js で実クリア検証（93テストパス）。ブロックエディタを if/while の
      条件選択・else 分岐に対応拡張し、実Chromeで while・if/else の組み立て→クリアをE2E確認
...
- [x] BGM追加（Web Audio 生成のループ、ミュート連動）、演出強化（マップに進捗バー・
      段階セクション見出し・「▶つぎ」ステージ強調）
- [x] ふりがな（js/ui/furigana.js：漢字ランを辞書でruby化、文脈例外つき）を
      タイトル/チュートリアル/ミッション/ヒント/マップに適用（テスト6件）
- [x] タブレット相当の 800px〜1280px 幅で表示確認（実Chromeでスクショ確認・4列に整列）
- [x] 実Chromeで回帰確認（プレイ→クリア→次、エラー0）、全124テストパス
- [x] PR作成 → CI/テスト確認 → マージ
